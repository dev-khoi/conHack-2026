...
export function RagAnswerCard({
  ...
  return (
    <Card
      className={cn(
        'relative overflow-hidden border-border/70 bg-card/75 shadow-xl backdrop-blur-xl transition-all duration-500 ease-out',
        ragLoading ? 'scale-[1.015] bg-card/85' : 'scale-100',
      )}>
      <div
        className={cn(
          'pointer-events-none absolute inset-0 z-0 rounded-2xl bg-background/20 backdrop-blur-md transition-opacity duration-500',
          ragLoading ? 'opacity-100' : 'opacity-0',
        )}
      />
      <CardHeader className='relative z-20 space-y-3 text-center'>
        <GradientText
      ...
      <CardContent className='relative z-20 space-y-4'>
        <section className='rounded-2xl border border-border/60 bg-muted/30 p-4 transition-all duration-500 sm:p-5'>
          ...
          <div className='text-sm leading-6 text-foreground'>
            <p className='whitespace-pre-wrap'>
              {ragLoading
                ...
                : hasAnswer
                  ? ragAnswer
                  : 'Ask a question and Aurora will answer from your memory database.'}
            </p>
          </div>
          ...
          <div className='relative'>
            ...
            <Input
              className='h-12 border-border/70 bg-background/90 pl-9 pr-24 focus-visible:ring-primary/30'
              value={query}
            ...
            {suggestedQueries.length === 0 ? (
              ...
              suggestedQueries.map((title) => (
                <Button
                  ...
                  variant='outline'
                  size='sm'
                  className='max-w-full overflow-hidden border-border/70 bg-background/70'
                  onClick={() => {
                    setQuery(title);
                    void runAskAi(title);
                  }}>
